    ...
    return (
        <div className="grid gap-5">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                            {loading ? "Loading users…" : `${filtered.length.toLocaleString()} registered student${filtered.length === 1 ? "" : "s"}`}
                        </p>
                    </div>
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                    {[
                        { icon: <Users size={16} />, label: "Total Users", value: users.length, tone: "bg-primary/10 text-primary" },
                        ...
                        { icon: <UserX size={16} />, label: "Inactive", value: users.filter((u) => u.status === "inactive" || !u.last_login_date).length, tone: "bg-amber-500/10 text-amber-600 dark:text-amber-400" },
                    ].map((s) => (
            ...
            <section className="overflow-hidden rounded-2xl border border-border bg-card">
                <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-4 border-b border-border bg-muted/30 px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    <span>User</span>
                ...
                <ul role="list" className="divide-y divide-border">
                    {loading ? (
                        ...
                        filtered.map((user) => (
                            <li key={user.id} className="grid grid-cols-[1fr_auto_auto_auto_auto] items-center gap-4 px-4 py-3.5 transition hover:bg-primary/[0.02]">
                                ...
                                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${user.role === "admin" ? "bg-indigo-500/10 text-indigo-600 dark:text-indigo-400" : "bg-muted text-muted-foreground"}`}>
                                    {user.role}
                                </span>
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-xs font-medium text-red-600 transition hover:border-red-300 hover:bg-red-50 dark:hover:bg-red-500/10"
                                >
